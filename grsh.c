...
bug report: occasionaly after using a long command, spaces that were used in the
list are set to '' instead of (null)
...
void shellMain(char *inputStr){
    ...
    memset(inputStr, 0, (sizeof(inputStr[0]) * 5000));
    memset(parsedArgs, 0, (sizeof(parsedArgs[0]) * 100));
    memset(parsedRedirectArgs, 0, (sizeof(parsedRedirectArgs[0]) * 100));
    memset(redirectFile, 0, (sizeof(redirectFile[0]) * 1));
}
